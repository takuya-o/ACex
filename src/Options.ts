...
import { BackgroundMsgCmd, Configurations, License, LicenseStatus } from './Types'
...
class Options {
  ...
  private displayLicenseStatus(reload = true) {
    chrome.runtime.sendMessage(
      { cmd: BackgroundMsgCmd.GET_LICENSE },
      // eslint-disable-next-line max-lines-per-function
      (license: License) => {
  ...
  private assignEventHandlers() {
    ...
    document.getElementById('check_license_by_chrome_web_store')!.onclick = this.onClickSave.bind(this)
  ...
  private restoreConfigurations() {
    chrome.runtime.sendMessage({ cmd: BackgroundMsgCmd.GET_CONFIGURATIONS }, (config: Configurations) => {
      this.restoreConfigurationsDisplayOptions(config)
  ...
  private restoreConfigurationsDisplayOptions(config: Configurations) {
    ...
    this.getInputElement('check_license_by_chrome_web_store').checked = config.useLicenseInfo
  ...
  private containsPermissions(
    lastPermission: boolean,
    permissions: chrome.permissions.Permissions,
    cb: (permission: boolean) => void,
  ) {
    //Permissionsとstoreされたconfigを同期して正しい表示にする
  ...
  private onClickSave(_evt: Event) {
    ...
    const forumMemoryCacheSize = parseInt(this.getInputElement('forum_memory_cache_size').value, 10)
    ...
    this.setPermissions(
      this.lastUseLicenseInfo,
      useLicenseInfo,
      Options.OPTIONAL_PERMISSION_IDENTITY,
