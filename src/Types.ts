...
interface BackgroundResponseName {
  name: string,
}
interface BackgroundResponseUrl {
  url: string,
}
interface BackgroundResponseForum {
  forum: Forum|undefined,
}
interface BackgroundResponseSession {
  userID: string,
  sessionA: string,
...
type BackgroundResponse = BackgroundResponseName|BackgroundResponseUrl|BackgroundResponseForum|
  BackgroundResponseSession|TextDetectionResult|License|Configurations|FontData|UrlString|TabId
...
//jsPDF用
interface PDFprops { w:number, h:number, mx:number, my:number, iw:number, ih:number }
...
interface ChromeWebStoreLicense {
...
interface ForumContents { //サーバからくるForum中身
  title: { value:string },
  updated: string, //Date,
  start: string, //Date,
  end: string, //Date,
...
//グラフ作成
interface CountingDatum { //ID毎の各週の投稿数カウント
  d:number[], //各週の投稿数
  uuid: string,
  name: string,
}

interface ChartData { //グラフデータ
