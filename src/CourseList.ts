...
class CourceList {
  ...
  private createList() {
    ...
    params.append("act", "1")
    CourseCommon.fetchJSON("https://aircamp.us/svlAC/GetCourseList?", params).then((json) => {
    ...
    //     MessageUtil.messsage( MessageUtil.getMessage(["program_list", "loding_fail"] ) + textStatus )
  ...
  private programItemList(programList:ProgramList, cmd:string) {
    ...
    programs.forEach( (program) => {
      ...
      //}
      for(const key in items) { //courseID順に並べる
        if ( key ) {
  ...
  private openCourseItem(courseID:string) {
    try {
      ...
      CourseCommon.fetchJSON("https://aircamp.us/svlAC/GetCourseItemList?", params).then((json) => {
        ...
        MessageUtil.message( `${MessageUtil.getMessage(["course_list", "logind_fail", "id"])}${courseID} ${error.name}:${error.message}` )
