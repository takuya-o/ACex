...
class CountResult {
  ...
  constructor() {
    window.addEventListener('load', (_evt: Event) => {
      ...
      chrome.runtime.sendMessage({ cmd: BackgroundMsgCmd.GET_SESSION }, (session: BackgroundResponseSession) => {
        //start
        if (!session?.userID || !session?.sessionA) {
          //sessionがnullのときが有る なんとTabHanderが返信している
        ...
        if (!fid) {
          ...
          chrome.runtime.sendMessage(
            { cmd: BackgroundMsgCmd.GET_FORUM_CACHE, fid: +fid },
  ...
  private createContents(fid: string, forceLoad: boolean, forum: Forum | undefined) {
    ...
    ;({ doing, forceLoad } = this.createContentsSetupReloadButton(forum, doing, isSaveContentInCache, forceLoad))
    if (!forceLoad && !doing && forum) {
      ...
      forum = this.createContentsByFetchData(fid, isSaveContentInCache, forum, forceLoad)
    }
  }

  private createContentsSetupReloadButton(
    forum: Forum | undefined,
    doing: boolean,
    isSaveContentInCache: boolean,
    forceLoad: boolean,
  ) {
    if (forum) {
    ...
    return { doing, forceLoad }
  }

  private createContentsByFetchData(
    fid: string,
    isSaveContentInCache: boolean,
    forum: Forum | undefined,
    forceLoad: boolean,
    ...
    CourseCommon.fetchJSON('https://aircamp.us/svlAC/GetForumContents?', params)
      .then(json => {
      ...
      .catch(error => {
        ...
        MessageUtil.message(
          MessageUtil.getMessage(['discussion_data', 'loding_fail']) + `${fid} ${error.name}:${error.message}`,
        )
        dataLayer.push({
          event: `Failure-GetForumContents ${error.name}:${error.message}`,
        })
      })
    return forum
  }

  private createCache(json: ForumContents, fid: string) {
    //サーバレスポンスからキャッシュを作る
    //発言一覧から 発言回数取得
    ...
    this.createCacheEntry(entries, forum)
    //キャッシュ記録
    ACexMessageSender.send(
      { cmd: BackgroundMsgCmd.SET_FORUM_CACHE, forum }, //非同期で登録
      () => {},
    ) // tslint:disable-line: no-empty
    this.downloadEnable(forum)
    return forum
  }
  private createCacheEntry(entries: ACEntry[], forum: Forum) {
  ...
  private getStartTime(title: string, startDate: Date) {
    // 開始日時を返す(タイトルがRTCOSのときは補正付き)
    // tslint:disable-next-line: no-shadowed-variable
    let start = startDate.getTime()
    if (title.indexOf('RTOCS') !== -1) {
      //RTOCSだった
      const days = startDate.getDay()
      if (days === 0 || days === 7) {
        //スタートが日曜日だったら20:00にする RTOCS補正
  ...
  private getMinPost(title: string) {
    // 必要発言数を返す(タイトルがRTCOSのときは補正付き)
    let minPost = 1 //最低投稿数 普通は週に1件 2件もあるので黄色
    if (title.indexOf('RTOCS') !== -1) {
      //RTOCSだった
      if (title.indexOf('振り返り') === -1) {
        //振り返り以外のRTOCSだった
  ...
  private calcWeeklyData(forum: Forum) {
    ...
    const start = this.getStartTime(forum.title, new Date(forum.start))
    forum.minPost = this.getMinPost(forum.title)
    const data = new Array<CountingDatum>()
    ...
    for (const entry of forum.entry) {
      if (entry && !entry.deleted) {
        //削除されていないものだけ収集
        ;({ minWeek, maxWeek, index } = this.caclWeeklyDataEntry(
          entry,
          forum,
          start,
          minWeek,
  ...
  private caclWeeklyDataEntry(
    ...
    if (relation === 0 || forum.entry[relation]!.uuid !== uuid) {
      ...
      if (isNaN(uuidIndex)) {
        ...
        if (isNaN(data[uuidIndex]?.d[week]!)) {
  ...
  private fillAuthorCache(i: number = 0, data: Array<ACEntry | CountingDatum>, sendResponse: () => void) {
    if (i % (data.length / 10) === 0) {
      console.log('fillAuthorCache() Entry=', i)
    } //AuthorNameCache更新 ログはEntry数/10ごとに出す。
    if (i < data.length) {
      if (data[i] && !CountResult.authorNameCache[data[i]!.uuid]) {
        //ACentryのdata[0]はempty  data[i]は必ずある
  ...
  private createTableMain(forum: Forum) {
    ...
    const finishMessages: string[] = /* グラフ出るよ this.noCharts ? ["count_finish", "no_charts", "id"] :*/ [
      'count_finish',
      'id',
    ]
    ...
    for (const uuid in counter) {
      //ここでuuidの型が強制的にstringになるよ 実はuuidはnumber
      if (uuid) {
        ranking.push({
          name: CountResult.authorNameCache[uuid]!,
          count: counter[uuid]!,
          deleted: deleted[uuid]!,
          reply: reply[uuid]!,
          ownReply: ownReply[uuid]!,
          replied: replied[uuid]!,
          uuid,
        })
      }
    }
  ...
  private createTableHTML(ranking: Ranking[], chartData: ChartData, forumMinPost: number) {
    ...
    ranking.forEach((rank, i) => {
      ...
      for (const data of chartData.data) {
        //一応、週テータもソートしてあるけどuuidでマッチを探す
        // tslint:disable-next-line: triple-equals
        if (rank.uuid == data.uuid) {
          //rank.uuidはstringなので、data.data[j]!.uuidはnumberなので注意
          d = data.d
          break
        }
      }
      for (let week = chartData.minWeek; week <= chartData.maxWeek; week++) {
        ...
        td.setAttribute('align', 'right')
        if (redClass !== '') {
          td.setAttribute('class', redClass)
        }
        td.innerText = weekCount.toString()
        tr.appendChild(td)
      }
      // item = item + '</tr>';
